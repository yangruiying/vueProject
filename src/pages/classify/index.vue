  ...
  <div class="container">
    <van-tabs v-model="active">
      <van-tab title="选车">
        ...
        <van-grid :column-num="4">
          <van-grid-item
            ...
            :icon="value.carIcon"
            :text="value.name"
            @click="toDetail(value.sortId, value.name)"
          />
        </van-grid>
        <van-grid :column-num="4">
          <van-grid-item
            @click="
              $router.push({ path: 'classifySearch', query: { value3: 1 } })
            "
          >
            <el-radio v-model="firstPay" :label="0.3" border size="small"
              >5-8万</el-radio
            >
          </van-grid-item>
          <van-grid-item
            @click="
              $router.push({ path: 'classifySearch', query: { value3: 2 } })
            "
          >
            <el-radio v-model="firstPay" :label="0.3" border size="small"
              >8-15万</el-radio
          ...
          <van-grid-item
            @click="
              $router.push({ path: 'classifySearch', query: { value3: 3 } })
            "
          >
            <el-radio v-model="firstPay" :label="0.3" border size="small"
              >15-20万</el-radio
          ...
          <van-grid-item
            @click="
              $router.push({ path: 'classifySearch', query: { value3: 4 } })
            "
          >
            <el-radio v-model="firstPay" :label="0.3" border size="small"
              >20-30万</el-radio
        ...
        <van-grid :column-num="4">
          <van-grid-item
            @click="
              $router.push({ path: 'classifySearch', query: { value1: 0 } })
            "
          >
            <el-radio v-model="firstPay" :label="0.3" border size="small"
              >轿车</el-radio
          ...
          <van-grid-item
            @click="
              $router.push({ path: 'classifySearch', query: { value1: 1 } })
            "
          >
            <el-radio v-model="firstPay" :label="0.3" border size="small"
              >SUV</el-radio
          ...
          <van-grid-item
            @click="
              $router.push({ path: 'classifySearch', query: { value1: 2 } })
            "
          >
            <el-radio v-model="firstPay" :label="0.3" border size="small"
              >跑车</el-radio
          ...
          <div v-for="(item, index) in first" :key="index">
            ...
            <div v-for="(item, index) in item.firstSortList" :key="index">
              <van-cell
                ...
                :icon="item.carIcon"
                @click="toDetail(item.sortId, item.name)"
              />
            </div>
          </div>
        </van-index-bar>
      ...
      <van-tab title="二手车" v-if="$route.query.toPath == null">
        <sencondCarList bannerShow="false">
...
export default {
  ...
  mounted() {
    this.getFirstSort(), this.getGoods(), this.getTopCar();
  },
  watch:{
    isLocal(value){
      if (value) {
        this.cityName = sessionStorage.getItem("city")
      }else{
        this.cityName = ""
      }
      this.getGoods();
  ...
  methods: {
    ...
    getGoods() {
      req2("getGoods", { type: 2 }).then((data) => {
    ...
    toDetail(id, name) {
      this.$router.push({
        ...
        query: {
          id: id,
          name: name,
          toPath: this.$route.query.toPath,
